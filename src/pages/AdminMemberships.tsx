...
const AdminMemberships = () => {
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (passwordInput === "teritorial2" || passwordInput === "pastorokrah1") { // Existing password check
      // NEW: Check page access after successful password entry
      fetch("/api/admin/page-access")
        .then(res => res.json())
        .then(data => {
          if (data.success && data.settings['admin-memberships'] === false) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-green-950/5">
      {isPasswordProtected ? (
        // Password Gate Modal
            ...
            <Card className="w-full max-w-md bg-slate-900 text-white shadow-2xl border-0">
              ...
              <CardContent>
                <p className="text-sm text-red-200 mb-4">
                  Access to the Memberships Manager has been temporarily disabled by the Master Administrator.
